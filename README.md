
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=blur&height=220&color=0:950606,100:60A5FA&text=SUMUKH%20S&fontSize=55&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Backend%20Engineer%20•%20AI%20Systems%20•%20Full%20Stack%20Developer&descAlignY=58&descSize=18" />
</p>

<h3 align="center">
Building scalable backend systems, AI-powered applications, and developer tools.
</h3>

<p align="center">
   <a href="https://sumukh-s.vercel.app/">
    <img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white" />
  </a>
  <a href="mailto:sumukh.codes@gmail.com">
    <img src="https://img.shields.io/badge/Email-181717?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://linkedin.com/in/sumukh-s-26089b245">
    <img src="https://img.shields.io/badge/LinkedIn-181717?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://github.com/Sumukh-S">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  <a href="https://leetcode.com/">
    <img src="https://img.shields.io/badge/LeetCode-181717?style=for-the-badge&logo=leetcode&logoColor=FFA116" />
  </a>
</p>

---

## 👋 About Me

- 💼 Software Engineering Intern at **Endava**
- 🎓 Information Science & Engineering Undergraduate (2022–2026)
- 🤖 Passionate about **Backend Engineering**, **System Design**, and **Agentic AI**
- 🌱 Currently building **MiniOrch**, a hand-rolled multi-agent orchestration engine
- 💬 Always happy to discuss Java, Go, Python, AI, and scalable systems

---

## 🚀 Featured Projects

#### [MiniOrch — Multi-Agent Orchestration Engine](https://miniorch.up.railway.app/)

A workflow orchestration engine built from scratch without LangChain or LangGraph.

**Highlights**

- DAG-based execution engine
- Parallel agent execution
- Retry & failure propagation
- Live execution monitoring
- Visual workflow builder

**Tech Stack**

`Python` • `FastAPI` • `TypeScript` • `Next.js` • `WebSockets`

---

#### [Mental Health Community Platform](https://github.com/Sumukh-S/TheraConnect)

AI-assisted mental health platform with human-in-the-loop verification.

**Highlights**

- AI-powered recommendations using Gemini
- REST API architecture
- Authentication & community features
- Responsive and accessible UI

**Tech Stack**

`MongoDB` • `Express` • `React` • `Node.js` • `Gemini API`

---

#### Anemia & Malnutrition Detection 🔒

Machine Learning pipeline for non-invasive childhood malnutrition detection using salivary biomarkers.

**Highlights**

- Data preprocessing
- Feature engineering
- Model training
- Classification pipeline

📄 Published at [**IEEE COSMIC 2025**](https://ieeexplore.ieee.org/document/11380947/)

---

## 💼 Experience

#### Software Engineering Intern
**Endava India Pvt. Ltd.** • *Jan 2026 – May 2026*

- Designed the database schema and Java backend for a version-control system
- Built frontend integration with REST APIs
- Implemented CI/CD pipelines
- Worked in Agile/Scrum development environments

---

#### Community Lead
**Information Science Student Developer Club**

- Led 20+ student developers
- Mentored project teams and open-source contributors

---

## 🛠 Tech Stack 

#### Languages

`Java` `Python` `Go` `TypeScript` `JavaScript` `SQL` `C++`

#### Backend

`Spring Boot` `FastAPI` `Flask` `Node.js` `Express.js`

#### Frontend

`React` `Next.js` `Tailwind CSS` `HTML5` `CSS3`

#### Databases

`PostgreSQL` `MongoDB` `MySQL` `Redis`

#### AI & Machine Learning

`Machine Learning`
`Agentic AI`
`Gemini API`
`Prompt Engineering`

#### Tools

`Git`
`GitHub`
`Docker`
`Postman`
`CI/CD`
`Agile`
`Claude Code`
`Vercel`
`azure`


---

## 🏆 Achievements

- 📄 Published & Presented at **IEEE COSMIC 2025**
- 💻 LeetCode Rating — **1752**

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Sumukh-S&theme=transparent&hide_border=true" />
</p>

---

## 📜 Certifications

- Oracle Cloud Infrastructure 2025 Certified Data Science Professional
- Java with DSA & System Design
- Postman Student Expert
- HackerRank React (Basic)

---

## 📫 Let's Connect

<p align="center">
  <a href="mailto:sumukh.codes@gmail.com">
    <img src="https://img.shields.io/badge/Email-60A5FA?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://linkedin.com/in/sumukh-s-26089b245">
    <img src="https://img.shields.io/badge/LinkedIn-60A5FA?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://github.com/Sumukh-S">
    <img src="https://img.shields.io/badge/GitHub-60A5FA?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

<p align="center">
<i>Always open to collaborating on backend engineering, distributed systems, and AI-powered products.</i>
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=120&section=footer&color=0:950606,100:60A5FA"/>
</p>

